...
def get_legal_moves(puzzle):
    ...
    # Check if the empty box can move left
    if empty_index % 3 != 0:
        legal_moves.append('left')

    # Check if the empty box can move right
    if empty_index % 3 != 2:
        legal_moves.append('right')

    # Check if the empty box can move up
    if empty_index >= 3:
        legal_moves.append('up')

    # Check if the empty box can move down
    if empty_index < 6:
        legal_moves.append('down')

    return legal_moves

def shuffle_puzzle(puzzle, moves):
    puzzle = list(puzzle)

    for _ in range(moves):
        legal_moves = get_legal_moves(puzzle)
...
# adds the 0 to the state(i.e x) if it is starting from zero
...
def getChildren(state):
    children = []
    idx = state.index('0')
    i = int(idx / 3)        # returns the column number
    j = int(idx % 3)        # returns the row number
    for x in range(0, 4):
        nx = i + dx[x]
        ny = j + dy[x]
        nwIdx = int(nx * 3 + ny)
